    ...
    <div >
      <div className="flex bg-white dark:bg-transparent rounded-2xl m-1 flex-1 flex-col gap-4 p-4 pt-0">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            ...
            >
              <Plus size={16} className="ml-1" /> {t("categoriesPage.createCategory")}
            </Button>
            <Button className="bg-[#252525] border-0 md:mb-0 rounded-2xl text-white/80 hover:text-white">
              {t("categoriesPage.howToCreateCategories")} <SquarePlay size={16} />
            </Button>
          </div>
        </div>

        {categories.length === 0 ? (
          <Empty className={" "} Name={t("categoriesPage.title").toLowerCase()} />
        ) : (
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge
                          ...
                          `}
                        >
                          {category.isActive ? t("categoriesPage.active") : t("categoriesPage.inactive")}
                        </Badge>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        <span className="text-sm font-medium text-muted-foreground">
                          {category.productCount} {t("categoriesPage.products")}
                        </span>
                      </div>
                    </div>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("categoriesPage.editCategory")}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="name">
                  {t("categoriesPage.categoryNameEnglish")}
                </Label>
                <Input
                  id="name"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  placeholder={t("categoriesPage.categoryNamePlaceholderEn")}
              ...
              <div className="space-y-2">
                <Label htmlFor="nameAr">
                  {t("categoriesPage.categoryNameArabic")}
                </Label>
                <Input
                  id="nameAr"
                  value={formData.nameAr}
                  onChange={(e) => setFormData({ ...formData, nameAr: e.target.value })}
                  placeholder={t("categoriesPage.categoryNamePlaceholderAr")}
                  className="rounded-xl"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl border bg-muted/50">
              <div className="space-y-0.5">
                <Label htmlFor="isActive" className="text-base">
                  {t("categoriesPage.activeStatus")}
                </Label>
                <p className="text-sm text-muted-foreground">
                  {t("categoriesPage.enableCategory")}
                </p>
              </div>
              <Switch
                id="isActive"
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t("categoriesPage.lowerNumbersFirst")}
              </p>
            </div>
          </div>
